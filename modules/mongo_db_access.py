...
class mongoDB(): 
    ...
    def query_user(self):
        user_list =[ i['user_name'] for i in self.col_user.find({'user_type':'uploader'})]
        return user_list
    
    def query_bricks(self):
        bricks = list(self.col_bricks.find({"brick_dict":{"$exists":True}}, {'_id': 0}))
        return loads(dumps(bricks))
    
    def query_creations(self):
        creations = list(self.col_creations.find({"creation_name":{"$exists":True}}, {'_id': 0}))
        return loads(dumps(creations))
    
    def query_one_creations(self,creation_id):
        creations = self.col_creations.find_one({"creation_id":creation_id}, {'_id': 0})
        return creations

    def query_creation_ratings(self,creation_id):
        ratings = list(self.col_ratings.find({"creation_id":creation_id}, {'_id': 0}))
